...
10. **Toda key en data.json debe estar declarada en config.yml.** Si data.json tiene una key que el config NO conoce (e.g. `i18n.labels.*` con sub-objects `{es, en}`), Decap CMS muestra esos objects como `Map { "es": "...", "en": "..." }` raw en el form y agrega errores fantasmas a `fieldsErrors`. **Regla:** todo lo que esté en data.json debe tener su widget. UI labels internos que no son editables (ON SALE, Archive, etc.) van **hardcoded en JS** (constante `UI_LABELS` en `main.js`) y NO en `data.json`.
11. **i18n + required: sub-fields ES/EN deben matchear el `required` del parent.** Si un field i18n es `widget: object` con `required: false` (parent opcional, ej. `sections[].title` que el marquee no tiene), los sub-fields `es` y `en` adentro **también** deben tener `required: false`. Decap valida sub-fields independientemente del parent: si quedan vacíos cuando el parent está ausente, dispara `PRESENCE` errors fantasmas → `persistEntry` rechaza con `Promise.reject()` SIN argumento, browser muestra "Uncaught (in promise) undefined" sin contexto. Para fields i18n obligatorios (tagline, role, chapters[].item, etc.), ambos sub-fields siguen required: true (default) — el editor debe llenar ES + EN ambos.
12. **Cuando el persist falla con `Uncaught (in promise) undefined`** sin más info, parchear `Promise.reject` para capturar stack trace y leer `state.entryDraft.fieldsErrors` del Redux store via React fiber walking — eso identifica el field exacto que falla validación (con `type`: PRESENCE/PATTERN/etc. y `parentIds`). Script de diagnóstico vive en `public/admin/index.html` y se controla con `localStorage.cms_debug = "off"` para silenciarlo.
